...
const REQUEST_ARTICLE = 'REQUEST_ARTICLE',
  ...
  fetchArticle = slug => (dispatch, getState) => {
...
const article$1 = (state = {}, action) => {
    switch (action.type) {
      case REQUEST_ARTICLE:
        return { ...state, slug: action.slug, failure: !1, isFetching: !0 };
      case RECEIVE_ARTICLE:
        return { ...state, data: action.data, failure: !1, isFetching: !1 };
      case FAIL_ARTICLE:
        return { ...state, failure: !0, isFetching: !1 };
      default:
        return state;
    }
  },
  idSelector = state => state.article.slug,
  itemSelector = state => state.article.data,
  articleSelector = createSelector(idSelector, itemsSelector, itemSelector, (slug, data, item) => {
    return (data && data[slug]) || data || item;
  });
var article$2 = { article: article$1, articleSelector: articleSelector };
store.addReducers({ article: article$1 });
class TSDetail extends connect(store)(LitElement) {
  ...
  render() {
    ...
    updateMetadata({ title: title, description: title, image: thumbnail });
    return html`
...
export {
  article as $article,
  article$2 as $article$1,
  tsArticle as $tsArticle,
  tsStyleWp as $tsStyleWp,
  FAIL_ARTICLE,
  RECEIVE_ARTICLE,
  REQUEST_ARTICLE,
  TsWordPressStyles,
  ...
  articleSelector,
  fetchArticle,
  fetchArticle as fetchArticle$1,
};
