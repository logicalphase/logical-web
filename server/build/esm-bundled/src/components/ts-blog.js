import{isPrimitive,directive,CDN_HOST_URL,LitElement,css,html,unsafeCSS,PageViewElement,connect,repeat,updateMetadata,formatDistance,unsafeHTML,Twitter,Linkedin,Facebook,Calendar,store,fetchArticles,refreshPage,articles,itemListSelector,SharedStyles,TsButtonStyle,TsElevationStyle,TsTypographyStyle,TsGridStyle,TsLayoutStyle,TsTheme}from"./ts-app.js";export{fetchArticles}from"./ts-app.js";export{refreshPage}from"./ts-app.js";const _state=new WeakMap,_infinity=2147483647,until=directive((...args)=>part=>{let state=_state.get(part);if(state===void 0){state={lastRenderedIndex:_infinity,values:[]};_state.set(part,state)}const previousValues=state.values;let previousLength=previousValues.length;state.values=args;for(let i=0;i<args.length;i++){// If we've rendered a higher-priority value already, stop.
...
var until$1={until:until};class TSSocial extends LitElement{static get styles(){return[css`
      :host {
        display: block;
        position: relative;
      }
    ...
    `]}render(){const{link,placeholder,_loaded,slug}=this;return html`
    <div id="placeholder">
...
window.open(link,"_blank","scrollbars=yes,resizable=yes,top=300,left=500,width=570,height=500")}_onSocialError(){if(!this.placeholder){return html`<p>Bummer. Unable to load social media share buttons.</p>`}}}customElements.define("ts-social",TSSocial);class TSItem extends LitElement{static get styles(){return[css`
      ...
      `]}render(){const{item}=this,id=item.id,title=item.title&&item.title.rendered,date=formatDistance(new Date(item.date),new Date),slug=item.slug,category=item.categories_names,excerpt=unsafeHTML(item.excerpt&&item.excerpt.rendered),date_prefix="Updated ",date_postfix=" ago.";return html`
...
window.open(link,"_blank","scrollbars=yes,resizable=yes,top=300,left=500,width=570,height=500")}}window.customElements.define("ts-item",TSItem);store.addReducers({articles});const cdnHost=unsafeCSS(CDN_HOST_URL);class TSBlog extends connect(store)(PageViewElement){static get styles(){return[TsButtonStyle,TsGridStyle,TsTypographyStyle,TsLayoutStyle,TsTheme,TsElevationStyle,SharedStyles,css`
      :host {
        display: block;
        padding: 0px;
      }
...
stateChanged(state){this._query=state.articles.query;this._data=itemListSelector(state);this._showOffline=state.app.offline&&state.articles.failure}}window.customElements.define("ts-blog",TSBlog);var tsBlog={fetchArticles:fetchArticles,refreshPage:refreshPage};export{tsBlog as $tsBlog,until$1 as $until,until};