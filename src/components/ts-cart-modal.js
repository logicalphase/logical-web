...
import { IronOverlayBehaviorImpl } from '@polymer/iron-overlay-behavior/iron-overlay-behavior';
...
import { TsButtonStyle } from './ts-button-style.js';
const TsButtonStyleTemplate = document.createElement('template');
TsButtonStyleTemplate.innerHTML = TsButtonStyle.strings[0];

class TSCartModal extends connect(store)(mixinBehaviors(
  [IronOverlayBehaviorImpl], PolymerElement)) {
  static get template() {
    return html`
    ${TsButtonStyleTemplate}
    <style>
      :host {
        display: block;
        position: fixed;
...
customElements.define('ts-cart-modal', TSCartModal);