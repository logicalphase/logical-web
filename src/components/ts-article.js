...
class TSDetail extends connect(store)(PageViewElement) {
  _render({_item, _lastVisitedListPage, _showOffline}) {
    ...
    const date = formatDistance(new Date(info.timestamp), new Date());
    const thumbnail = info.photo;
    ...
    return html`
      ...
      <section hidden?="${_showOffline}">
        <div class="info">
          <div class="cover" hero>
            ...
            <div class="info-item" hidden?="${!author}">By ${author} - Published: ${date} agos.</div>
          </div>
        </div>
        <div class="desc">
          <h3>Description</h3>
          ${unsafeHTML(info.body || info.subtitle || 'None')}
        </div>
      ...
      <ts-offline hidden?="${!_showOffline}" on-refresh="${() => store.dispatch(refreshPage())}"></ts-offline>
    `;
  }
  ...
  _stateChanged(state) {
    this._item = articleSelector(state);
